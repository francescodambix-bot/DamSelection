...
async function getSessionCookies(domain) {
  ...
  // getSetCookie() gestisce correttamente i cookie multipli; fallback al parsing manuale se non disponibile
  let cookiePairs = [];
  ...
  // Vinted a volte manda access_token_web due volte (un reset vuoto + il valore vero):
  // prendiamo l'ultima occorrenza con un valore non vuoto, non semplicemente la prima trovata
  ...
  try {
    ...
    for (const q of queries) {
      ...
      for (let page = 1; page <= pages; page++) {
        ...
        rawItems.push(...items);
        await new Promise((r) => setTimeout(r, 700)); // ritmo prudente anche lato server
      }
